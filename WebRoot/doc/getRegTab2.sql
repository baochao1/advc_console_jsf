ALTER FUNCTION getRegTab2(@userID      INT,
                                 @strategyID  INT,
                                 @studyCourse VARCHAR(130),
                                 @planID      INT,
                                 @isRetain    INT)
RETURNS @tabA TABLE(
  ...
  BEGIN
      ...
      IF @isCourseOrder = 0
        BEGIN
            ...
            UPDATE @tmp2
            SET    countPercent = CONVERT(NUMERIC(3, 1), minPercent / countPercent)
            FROM   @minCount
            WHERE  minPhaseNo = countPhaseNo

            UPDATE @tmp
            SET    chpSequence = chpSequence * countPercent * 10
            FROM   @tmp2
